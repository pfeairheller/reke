...
init(_) ->
  ets:new(migrations, [named_table, ordered_set]),
...
process_files([File | Rest], Dir, Processed) ->
  ...
  case ets:lookup(migrations, NumStr) of
    [_] ->
      io:format("Migration ~s already complete.~n", [NumStr]),
      process_files(Rest, Dir, Processed);
    [] ->
      case file:consult("db/migrate/" ++ File) of
        {ok, Tokens} ->
          io:format("Running Migration ~s.~n", [NumStr]),
...
ensure_migration_table() ->
  ...
  case reke_connection:squery(reke, "SELECT version FROM schema_migrations ORDER BY version ASC") of
    {ok, _Columns, Rows} ->
      ets:insert(migrations, lists:map(fun(E) -> {binary:bin_to_list(element(1, E))} end, Rows)),
      {ok, Rows};
    {error, Error} ->
      {error, Error}
  end.
